...
async def compare(ctx, plane1, plane2, plat = 'pc'):
    if paused == False:
        counter('compare')
        succ1 = False
        succ2 = False
        ...
        try:
            ...
            pass
        ps = addspaces(ac1[8], 20, plat)
        cs = addspaces(ac1[6], 16, plat)
        rs = addspaces(ac1[2], 20, plat)
        ss = addspaces(ac1[5], 18, plat)
        fs = addspaces(ac1[4], 5, plat)
        es = addspaces(ac1[3], 7, plat)
        
        fr = addspaces(f'{pro1[0]:,}', 9, plat)
        dr = addspaces(round(pro1[1], 2), 12, plat)
        pr = addspaces(f'{pro1[2]:,}', 12, plat)
        fe = addspaces(f'{pro1[3]:,}', 12, plat)
        de = addspaces(round(pro1[4], 2), 15, plat)
        pe = addspaces(f'{pro1[5]:,}', 15, plat)
