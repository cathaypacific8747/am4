...
async def routes(ctx, airportName, shortACname, targetDistance, flightsPerDay):
    ...
    if o == [] or range == -1:
        ...
        with open(f'data/dist1/{o[0]}.csv', newline='', encoding='utf-8-sig') as f:
            ...
            for f in filtered:
                if int(f[10]) > 0: # income is +ve
                    n = '\n          ' if isMobile else '' # mobile separator
                    val = '```yml\n'
                    try:
                        if needsStopover:
                            ...
                            if stop != None and stop != 'UNREACHABLE': # add stopover to display
                                val += f'Stopover: {stop[1]}, {n}{stop[2]}' + '\n'
                                val += f'          {stop[3]} | {stop[4]}' + '\n'
                            else:
                        ...
                        if isCargo:
                            val += f'  Demand: l({round(int(f[1]) * 0.5)}k), {n}h({f[2]}k)' + '\n'
                            val += f'  Config: l({int(f[11][0] * 100)}%), {n}h({int(f[11][1] * 100)}%)' + '\n'
                            val += f' Tickets: l(${f[12][0]}), {n}h(${f[12][1]})' + '\n'
                        else:
                            val += f'  Demand: y({f[1]}), {n}j({f[2]}), {n}f({f[3]})' + '\n'
                            val += f'  Config: y({f[11][0]}), {n}j({f[11][1]}), {n}f({f[11][2]})' + '\n'
                            val += f' Tickets: y(${f[12][0]:,d}), {n}j(${f[12][1]:,d}), {n}f(${f[12][2]:,d})' + '\n'
                        ...
                        embed.add_field(name=f'**{f[4]}**, {f[5]}', value=val, inline=False)
                        counter += 1
                if counter > 10:
                    break # stop the loop, as it has already displayed over 10 results
                    # just in case the possible routes are less than 10, the for loop would've finished already
            if counter == 0:
                embed.add_field(value='There are no profitable routes.')
            await ctx.send(embed=embed)
