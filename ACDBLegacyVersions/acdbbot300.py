...
async def on_reaction_add(reaction, user):
    ...
    if user != reaction.message.author and str(reaction.emoji) == '📱':
        ...
        await reaction.message.edit(embed = embed)
    elif user != reaction.message.author and str(reaction.emoji) == '<:cargo:687641910394748996>':
        msg = (f'```ml\n           Price: "{ac[8]} million"\n        Capacity: "{ac[6]},000 lbs"\n           Range: "{ac[2]} km"\n           Speed: "{ac[5]} km/h"\nFuel consumption: "{ac[4]} lbs/km"\n   CO2 emmisions: "{ac[3]} kg/1k lbs/km"```')
        embed.set_field_at(0, name = 'Brief Statistics (Cargo)', value = msg)
        await reaction.message.edit(embed = embed)
    ...
    if paused == False:
        ...
        if injection == False:
            try:
                ...
                cursor.execute(f"SELECT `manf`, `aircraft`, `rng`, `co2`, `fuel`, `spd`, `cap`, `model`, `cost`, `img` FROM `am4bot` WHERE `shortname` = '{plane}'")
                acdb.close()
        ...
        for ac1 in cursor:
            ...
            await message.add_reaction('🦵')
            await message.add_reaction('<:cargo:687641910394748996>')
...
async def compare(ctx, plane1, plane2):
    if paused == False:
        ...
        if injection == False:
            try:
                acdb.reconnect(attempts = 5, delay = 0.5)
                cursor.execute(f"SELECT `manf`, `aircraft`, `rng`, `co2`, `fuel`, `spd`, `cap`, `model`, `cost`, `img` FROM `am4bot` WHERE `shortname` = '{plane1}'")
            except mysql.connector.Error as error:
                await ctx.send(f'Database error. Contact <@243007616714801157> if this happens.```python\n{error}```')    
            for ac in cursor:
                ac1 = ac
...
async def search(ctx, value):
    if paused == False:
        ...
        try:
            acdb.reconnect(attempts = 5, delay = 0.5)
            cursor.execute(f"SELECT `manf`, `aircraft`, `rng`, `co2`, `fuel`, `spd`, `cap`, `shortname`, `cost` FROM `am4bot` WHERE `cost` <= {float(value)} ORDER BY `Cost` DESC LIMIT 10")
            acdb.close()
        except mysql.connector.Error as error:
            await ctx.send(f'Database error. Contact <@243007616714801157> if this happens.```python\n{error}```')
        embed = discord.Embed(title = f'Showing planes under **${value} Million**:', colour = discord.Colour(0x33b300))
        for ac in cursor:
