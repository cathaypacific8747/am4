...
@bot.command(help='Reports the current price, pinging PriceNofify.', usage='$price f<fuel price> c<co2 price>')
async def price(ctx, *costs):
    if ctx.message.channel.id == priceAlertId:
        furtherErrorInfo = f'See `$help price` for proper command usage in {bot.get_channel(botspamId).mention}.'
        try:
