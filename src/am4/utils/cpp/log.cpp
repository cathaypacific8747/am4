...
UserLog::Share::Share(const TimePoint& ts, float share) : ts(ts), share(share) {}

UserLog::Award::Award(const TimePoint& ts, const string& award) : ts(ts), award(award) {}
...
UserLog::UserLog(
    ...
    : id(id),
      ...
      cargo_reputation(cargo_reputation),
      founded(founded),
      logo(logo),
      share_log(share_log),
      awards(awards),
