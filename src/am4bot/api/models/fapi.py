...
FAPIReqRealism = Annotated[bool, Query(description="[Optional] **Whether to use realism mode** - defaults to `false` (easy) if not specified.")]
...
class FAPIReqUser():
    def __init__(
        ...
        wear_training: Annotated[int, Query(description="[Optional] **Wear training** - defaults to `0` if not specified.", ge=0, le=5)] = None,
        repair_training: Annotated[int, Query(description="[Optional] **Repair training** - defaults to `0` if not specified.", ge=0, le=5)] = None,
        l_training: Annotated[int, Query(description="[Optional] **L training** - defaults to `0` if not specified.", ge=0, le=6)] = None,
        h_training: Annotated[int, Query(description="[Optional] **H training** - defaults to `0` if not specified.", ge=0, le=6)] = None,
        fuel_training: Annotated[int, Query(description="[Optional] **Fuel training** - defaults to `0` if not specified.", ge=0, le=3)] = None,
        co2_training: Annotated[int, Query(description="[Optional] **CO2 training** - defaults to `0` if not specified.", ge=0, le=5)] = None,
        fuel_price: Annotated[int, Query(description="[Optional] **Fuel price** - defaults to `700` if not specified.", ge=0, le=3000)] = None,
        co2_price: Annotated[int, Query(description="[Optional] **CO2 price** - defaults to `120` if not specified.", ge=0, le=200)] = None,
        accumulated_count: Annotated[int, Query(description="[Optional] **Accumulated fleet count**, used for marketing cost estimation - defaults to `0` if not specified.", ge=0, le=65535)] = None,
        fourx: Annotated[bool, Query(description="[Optional] **4x** - defaults to `false` if not specified. *Note*: if this is set in the aircraft options (e.g. `a388[x]`), it will take precendence.")] = None,
        income_loss_tol: Annotated[float, Query(description="[Optional] **Income loss tolerance** - defaults to `0.0` if not specified. During end-game, hub availability becomes an issue and you might want to cram in more aircraft per route, even if that means losing some income. By default, the algorithm will stop increasing the trips/day as soon as the income drops below the maximum: setting this to `0.2` means that it'll continue inflating until the income is 80% of the max before moving onto the next candidate. Raising this value often yields much better income per hub.", ge=0, le=1)] = None,
        load: Annotated[float, Query(description="[Optional] **Assumed aircraft load** - defaults to `0.87` if not specified (meaning 87% passenger filled: demand will be 'virtually' inflated by 1/0.87 = +14.9%.)", gt=0, le=1)] = None,
