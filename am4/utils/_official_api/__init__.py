...
from am4.utils.log import AllianceLog, UserLog
...
async def fetch_user(query: str | int, local_file: str | None) -> Tuple[Status, UserLog | None]:
    ...
    return ur.status, UserLog(
        ...
        online=u.online,
        share=u.share,
        shares_available=u.shares_available,
        shares_sold=u.shares_sold,
        ipo=u.ipo,
        fleet_count=u.fleet_count,
        routes=u.routes,
        alliance=u.alliance,
        ...
        cargo_reputation=u.cargo_reputation,
        founded=u.founded,
        logo=u.logo,
        share_log=[UserLog.Share(ts=sl.ts, share=sl.share) for sl in ur.share_log],
        awards=[UserLog.Award(ts=a.ts, award=a.award) for a in ur.awards],
        fleet=[],
        route_list=[],
    )
